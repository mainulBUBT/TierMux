...
export const SETTINGS_META: SettingMeta[] = [
  ...
  { key: 'agent.plannerGate', label: 'Planner gate', type: 'boolean',
    desc: 'Skip the planner step for confident, short, single-action requests — one less serial LLM round-trip before work starts. Multi-step-shaped requests always plan.' },
  ...
  { key: 'agent.terseReplies', label: 'Terse replies', type: 'boolean',
    desc: 'Instruct the model to answer tersely (no preamble, lead with the answer) — saves output tokens at the cost of some warmth.' },
