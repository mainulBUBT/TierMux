...
Before touching the loop — `src/agent/core/cline/clineEngine.ts` (the Cline `AgentRuntime` wiring; `engine.ts` was the pre-cline AI SDK loop and is deleted), `src/agent/core/cline/prepareTurn.ts`, or the completion-policy config — adding any "quality"/retry/detector logic, or editing the system prompt, read [docs/SIMPLE_CORE_RESET_2026-08-24.md](docs/SIMPLE_CORE_RESET_2026-08-24.md). Short version: the loop executes tools/models, preserves the ONE transcript, rotates providers, and recovers from provider failures with exactly ONE mechanical continuation (on the cline branch that continuation is the runtime's own — completion reminder, overflow recovery, ≤3 provider retries). It never judges answer quality, never detects "narration", never retries on weak-looking output. Any new guard needs a live repro cited in its comment — one targeted guard at a time, never a tower. Verify with `npm run test:e2e:foundation` (THE contract) plus `test:e2e:cline-engine` and the rest of the suites listed in that doc.
