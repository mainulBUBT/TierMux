...
const base = (): SelectionRationale => ({
  ...
  entries: [
    entry('opencode::muse-spark-1.2-contributor-free', true, 'task table (work) — serves this turn'),
...
ok('6. candidates walked past are relabelled as attempted',
   fixed.entries.find((e) => e.model === 'opencode::muse-spark-1.2-contributor-free')!.reason === 'task table (work) — tried first, failed over');
...
ok('11. chain[0] serving keeps "serves this turn"',
   happy.entries[0].reason === 'task table (work) — serves this turn' && happy.entries[0].selected);
